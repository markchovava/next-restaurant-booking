...
import { _contactUpdateAction } from '@/_api/_actions/ContactActions';
    ...
    const { 
       ...
       async function postData(e: React.FormEvent<HTMLFormElement>) {
            ...
            if (!validation.isValid) {
                ...
                const firstError = validation.errors.name || validation.errors.message ||
            ...
            const formData = {
                ...
                status: data.status,
                   ...
                   try {
                       const res = await _contactUpdateAction(data.id, formData);
                       if (res.status === 1) {
